...
def print_feats2(opt, test_data, f, level, t):
    feature_path = '{}/features/'.format(opt['path']['results'])
    os.makedirs(feature_path, exist_ok=True)
    img_f = Metrics.tensor2img(test_data["img"][0, :, :, :])
    Metrics.save_img(img_f, '{}/img_f.png'.format(feature_path))
    for i in range(len(f[level])):
        feat_img = Metrics.tensor2img(f[level][i][0, 0, :, :])  # uint8
        Metrics.save_feat(feat_img, '{}/feat_{}_level_{}_t_{}.png'.format(feature_path, i, level, t))
